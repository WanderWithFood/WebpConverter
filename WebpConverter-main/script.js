...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function resetAndClearSessionCache() {
        try {
            sessionStorage.clear();
            localStorage.removeItem('webp_converter_queue');
            localStorage.removeItem('filesQueue');
        } catch (e) {}

        state.filesQueue = [];
        state.selectedWebpId = null;

        if (fileInput) fileInput.value = '';
        const addMoreInput = document.getElementById('addMoreInput');
        if (addMoreInput) addMoreInput.value = '';

        if (queueCountBadge) queueCountBadge.textContent = '0 Files';
        if (queueValidBadge) queueValidBadge.textContent = '0 Valid';
        if (queueInvalidBadge) {
            queueInvalidBadge.textContent = '0 Unsupported';
            queueInvalidBadge.classList.add('hidden');
        }
        hideErrorBanner();
        ...
        if (unsupportedFilesList) unsupportedFilesList.innerHTML = '';
    }

    // Initialize clean state on page load and clear cache after session
    resetAndClearSessionCache();
    ...
    async function detectImageBySignature(file) {
        // Read first 64 bytes to detect common image signatures
        function readSlice(bytes) {
        ...
        try {
            const header = await readSlice(64);
            ...
            // AVIF: 'ftyp' at 4-7 and brand 'avif' at 8-11
            if (header[4] === 0x66 && header[5] === 0x74 && header[6] === 0x79 && header[7] === 0x70
                && header[8] === 0x61 && header[9] === 0x76 && header[10] === 0x69 && header[11] === 0x66) return 'image/avif';

            // SVG: try reading as text and look for '<svg'
            const textSlice = await new Promise((resolve, reject) => {
            ...
            if (typeof textSlice === 'string' && textSlice.toLowerCase().includes('<svg')) return 'image/svg+xml';
    ...
    async function validateFile(file) {
        ...
        // If browser provides a MIME type and it looks like an image, trust it
        if (file.type && (file.type.startsWith('image/') || SUPPORTED_MIMES.includes(file.type))) {
            return { isValid: true, errorMsg: null };
        }

        // Otherwise try to detect by binary signature (safer than trusting extension only)
        const detected = await detectImageBySignature(file);
        if (detected && (detected.startsWith('image/') || SUPPORTED_MIMES.includes(detected))) {
            return { isValid: true, errorMsg: null };
        }

        return {
            isValid: false,
            errorMsg: `Unsupported or invalid image file (${file.type || 'unknown'} / .${ext}).`
        };
    }
    ...
    async function processIncomingFiles(files) {
            ...
            if (item.isValid) {
                ...
                reader.onload = (e) => {
                    ...
                    img.onerror = () => {
                        item.isValid = false;
                        item.status = 'error';
                        item.errorMsg = 'Failed to load image data';
                        updateQueueUI();
                    };
        ...
        updateQueueUI();
        showToast(`Added ${newItems.length} file(s) to queue`, 'fa-solid fa-plus-circle');
    }
    ...
    function renderUnsupportedBanner(invalidItems) {
        ...
        invalidItems.forEach(item => {
            ...
            row.innerHTML = `
                ...
                <div style="display: flex; align-items: center; gap: 10px;">
                    ...
                    <button type="button" class="btn-icon-danger remove-unsupported-btn" data-id="${item.id}" title="Remove file">
        ...
        unsupportedFilesList.querySelectorAll('.remove-unsupported-btn').forEach(btn => {
    ...
    function renderStep4Results() {
        ...
        if (validDoneItems.length === 0) {
            if (downloadWebpBtn) downloadWebpBtn.classList.add('hidden');
            return;
        }

        if (!state.selectedWebpId || !validDoneItems.some(item => item.id === state.selectedWebpId)) {
            state.selectedWebpId = validDoneItems[0].id;
        }
        ...
        if (downloadWebpBtn) {
            downloadWebpBtn.classList.remove('hidden');
        }
        ...
        if (batchResultsContainer) {
            ...
            validDoneItems.forEach((item) => {
                const row = document.createElement('div');
                const isSelected = item.id === state.selectedWebpId;
                row.className = `batch-result-row${isSelected ? ' selected-result-row' : ''}`;
                ...
                row.innerHTML = `
                    ...
                    <div class="result-row-right">
                        ...
                        <button type="button" class="btn-icon-download" data-download-item-id="${item.id}" title="Download single WebP">
                ...
                row.addEventListener('click', (event) => {
                    if (event.target.closest('[data-download-item-id]')) return;
                    state.selectedWebpId = item.id;
                    renderStep4Results();
                });
